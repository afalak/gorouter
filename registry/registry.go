...
func NewRouteRegistry(logger zap.Logger, c *config.Config, reporter reporter.RouteRegistryReporter) *RouteRegistry {
	r := &RouteRegistry{}
	r.logger = logger
...
func (r *RouteRegistry) Register(uri route.Uri, endpoint *route.Endpoint) {
	t := time.Now()
	data := []zap.Field{zap.String("uri", uri.String()), zap.String("backend", endpoint.CanonicalAddr()), zap.Object("modification_tag", endpoint.ModificationTag)}
	...
	if pool == nil {
		...
		r.logger.Debug("uri-added", zap.String("uri", uri.String()))
	}
		...
		go func() {
			for {
				...
				case <-r.ticker.C:
					...
					r.logger.Debug("finished-pruning-droplets")
...
func (r *RouteRegistry) pruneStaleDroplets() {
	...
	r.byUri.EachNodeWithPool(func(t *container.Trie) {
		...
		if len(endpoints) > 0 {
			...
			r.logger.Info("pruned-route", zap.String("uri", t.ToPath()), zap.Object("endpoints", addresses))
		}
	})
}
